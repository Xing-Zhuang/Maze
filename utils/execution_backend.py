...
class VLLMRunner:
    ...
    def start_server(self) -> None:
        ...
        self.logger.info(f"  -> Starting vLLM server and model_path: {self.model_path} on GPUs {self.gpu_indices} at port {self.port}...")
        vllm_command = [
        ...
        try:
            ...
            self.logger.info(f"🚀 Launching vLLM for model '{model_display_name}' on GPUs {self.gpu_indices} at port {self.port}...")
            # --- 核心修改：重定向vLLM进程的输出到日志文件 ---
            log_dir = os.path.join(self.proj_path, "src", "agentos", "logs", "vllm_logs")
            os.makedirs(log_dir, exist_ok=True)
            self.logger.info(f"  -> vllm log_dir: {log_dir}")
            ...
            self.logger.info(f"  -> 📜 vLLM stdout log: {stdout_log_path}")
            self.logger.info(f"  -> 📜 vLLM stderr log: {stderr_log_path}")
            ...
            self.logger.error(f"❌ vLLM process Popen failed for model '{model_display_name}': {e}")
            if self.process: self.process.kill()

    def stop_server(self):
        if self.process:
            self.logger.info(f"🛑 Stopping vLLM server process group at port {self.port}...")
            try:
                # --- 核心修改2：终止整个进程组 ---
                ...
                self.logger.info(f"  -> Process group for port {self.port} terminated.")
            except (ProcessLookupError, TimeoutError) as e:
                 self.logger.warning(f"  -> Graceful shutdown failed ({e}), attempting forceful kill...")
                 try:
                     # 如果优雅关闭失败，则使用SIGKILL强制杀死
                     os.killpg(os.getpgid(self.process.pid), signal.SIGKILL)
                 except Exception as kill_e:
                     self.logger.error(f"  -> Forceful kill failed: {kill_e}")
...
class VLLMBackend(ExecutionBackend):
    ...
    def __init__(self, resource_manager, proj_path, model_folder, models_config_path):
        ...
        try:
            ...
            self.logger.info("✅ VLLMBackend: Model configuration loaded from 'models.json'.")
        except Exception as e:
            self.model_config = {}
            self.logger.error(f"😭 Failed to load model configuration from '{models_config_path}': {e}")
    ...
    def deploy(self, node_id: str, gpu_indices: List[int], model_name: str):
        ...
        self.logger.debug(f"  -> Assigning Actor name: {runner_name}")
        ...
        try:
            ...
            self.logger.info(f"⏳ VLLMBackend: Deployment initiated for '{model_name}' on Node {node_id[:6]}/GPUs {gpu_indices}.")
        
        except Exception as e:
            self.logger.error(f"❌ VLLMBackend deployment failed to initiate: {e}")
    ...
    def undeploy(self, node_id: str, gpu_indices: List[int]) -> bool:
        ...
        if runner_actor:
            try:
                self.logger.info(f"  -> 📩 Sending stop signal to VLLMRunner on node {node_id}...")
                # 尝试优雅地停止服务，但设置一个较短的超时时间
                ray.get(runner_actor.stop_server.remote(), timeout=15) 
                self.logger.info(f"  -> ✅ VLLMRunner on node {node_id} stopped gracefully.")
            except ray.exceptions.ActorDiedError:
                # 如果 Actor 在我们尝试停止它时死亡（很可能是因为节点被终止），
                # 我们可以认为卸载目标已达成，因为 Actor 确实消失了。
                self.logger.error(f"  -> ⚠️  VLLMRunner actor died during stop command (as expected if node is terminating). Continuing cleanup.")
            except ray.exceptions.ActorUnavailableError as e:
                # Actor 已经不可用（如 SIGTERM），通常是节点或进程被外部终止
                self.logger.error(f"  -> ⚠️  VLLMRunner actor unavailable (likely already terminated): {e}. Continuing cleanup.")
            except ray.exceptions.RayTaskError as e:
                # 捕获 Actor 内部在执行 stop_server 时发生的错误
                self.logger.error(f"  -> ❌ Error occurred inside the VLLMRunner's stop_server method: {e}")
            except Exception as e:
                # 捕获其他可能的异常，例如超时
                self.logger.error(f"  -> ❌ An unexpected error occurred during undeploy: {type(e).__name__} - {e}")
            finally:
                # 无论上面发生了什么，最后都要尝试 kill Actor 句柄。
                # 这能确保 Ray 清理其内部状态，释放资源。
                self.logger.error(f"  -> 💥 Killing actor handle to ensure resource release in Ray.")
            ...
            self.logger.info(f"  -> 🟢 Resources for node {node_id}, GPUs {gpu_indices} have been marked as FREE.")
            return True
        return False
    ...
    def is_server_ready(self, node_id: str, gpu_indices: List[int]) -> Tuple[bool, Optional[str]]:
        ...
        try:
            ...
            self.logger.debug(f"  -> ✅ Health check successful for {api_url}. Server is ready.")
            return True, api_url
            # --- 修复结束 ---
        except requests.exceptions.RequestException as e:
            # 只有在请求失败（如连接超时、DNS错误、非2xx状态码）时才会进入这里
            if 'api_url' in locals():
                 self.logger.debug(f"   -> [Debug] Health check for {api_url} failed: {type(e).__name__} - {e}")
            return False, None
...
class HuggingFaceBackend(ExecutionBackend):
    ...
    def deploy(self, node_id: str, gpu_index: int, model_name: str) -> Optional[Dict]:
        ...
        self.logger.info(f"✅ Preparing to run '{model_name}' with HuggingFaceBackend on Node {node_id[:6]}, GPU {gpu_index}.")
    ...
    def undeploy(self, node_id: str, gpu_index: int) -> bool:
        ...
        self.logger.info(f"✅ HuggingFaceBackend task finished on Node {node_id[:6]}, GPU {gpu_index}. Resources are auto-released.")
        return True