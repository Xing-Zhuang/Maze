...
def encode_image_bytes_base64(img_bytes: bytes) -> str:
    """[新增] 将图片字节流编码为base64字符串"""
    return base64.b64encode(img_bytes).decode('utf-8')

# =================================================================
#               ⬇️ 新增的 vLLM 图像批量查询函数 ⬇️
# =================================================================
...
def extract_vision_features(image_bytes: bytes) -> Dict[str, float]:
    """为单个图像提取VLM预测所需的特征，包括文本区域比例和文本块数量。"""
...
def query_vlm_batch(
    ...
    try:
        console.print(f"🔯 [bold green]开始加载VLM模型 (使用强制padding配置)... ({model_path})[/bold green]")
...
def task3_file_process(context):
    """Task 3: [已修改] 增强图像质量，标准化尺寸，并为VLM任务准备特征。"""
    try:
        ...
        for image_info in all_images:
            ...
            img = cv2.imdecode(img_array, cv2.IMREAD_COLOR)
            if img is None:
                print(f"❌ 解码失败，跳过: {image_info['file_name']}")
                continue
            ...
            if h > MAX_DIMENSION or w > MAX_DIMENSION:
                ...
                print(f"  -> 图像 '{image_info['file_name']}' 已从 {w}x{h} 缩放到 {new_w}x{new_h}")
            # --- 修改结束 ---
...
def task4a_vlm_process(context):
    """[Parallel Worker A] 使用VLM为前20%图片生成答案。"""
    task_id = "4a"
    try:
...
def task4b_vlm_process(context):
    """[Parallel Worker B] 使用VLM为第二个20%图片生成答案。"""
    task_id = "4b"
    try:
...
def task4c_vlm_process(context):
    """[Parallel Worker C] 使用VLM为第三个20%图片生成答案。"""
    task_id = "4c"
    try:
...
def task4d_vlm_process(context):
    """[Parallel Worker D] 使用VLM为最后40%图片生成答案。"""
    task_id = "4d"
    try:
...
def task4_merge_results(context):
    """[Merge] 合并来自task4a, 4b, 4c, 4d的并行处理结果。"""
    task_id = "4_merge"
    try:
        ...
        # 为了与原始task4的输出保持一致，我们在这里重新计算聚合特征
        ...
        return json.dumps({
            "dag_id": dag_id, "status": "success",
            "curr_task_feat": aggregated_image_features, # 保持与原task4一致
