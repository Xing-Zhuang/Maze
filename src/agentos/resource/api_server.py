...
def handle_inform()-> object:
    ...
    Expected input format:
    {
        ...
        "type": "io",     #io,cpu,gpu
        "cpu_num": 1,     #cpu类型才有
        "mem": 100,       #cpu,io类型才有，单位MB
        "gpu_mem": 1024   #gpu类型才有，单位MB
    }
    ...
    # NEW: 获取优先级信息
    priority = data.get("priority") # 如果ltf没传，则为None
        ...
        for actor_info in named_actors:
            ...
            if actor_name and actor_name.startswith("vllm_runner_"):
                try:
                    ...
                    # 远程调用Actor自己的stop_server方法，而不是用ray.kill()
