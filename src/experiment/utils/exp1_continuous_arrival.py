...
# ==========================================================================
#  Part 0: Helper Functions (新增clear_console)
# ==========================================================================
def clear_console():
    """清空终端屏幕，以实现动态刷新效果。"""
...
# ==========================================================================
#  Part 1: API Client (新增 get_status_text 方法)
# ==========================================================================
...
# ==========================================================================
#  Part 2: 统一的压测逻辑 (BenchmarkRunner - 已更新)
# ==========================================================================
    ...
    def _worker(self, worker_id: int):
        ...
        while not self.stop_event.is_set() or not self.request_queue.empty():
            ...
            try:
                ...
                # --- BUG FIX 3: 确保任务（无论成功失败）都从监控列表中移除 ---
                if unique_id:
                    with self.live_dags_lock:
                        if unique_id in self.live_dags_status:
                            del self.live_dags_status[unique_id]
                            self.completed_count += 1 # 计数已完成（包括成功或失败）
                if handle:
                    self.client.release(handle)
                time.sleep(0.01)

    # _log_result, run, _analyze_results 方法无需任何改动
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="统一的动态负载压测客户端。")
    parser.add_argument("--target_system", type=str, required=True, choices=['ours', 'autogen', 'agentscope', 'vllm'], help="要测试的目标系统 ('ours' 或 'autogen')。")
    parser.add_argument("--proj_path", default="/root/workspace/d23oa7cp420c73acue30/AgentOS", help="AgentOS项目根目录。")
    parser.add_argument("--seed", type=int, default=42, help="随机种子 (默认: 42)")
    parser.add_argument("--workers", type=int, default=20, help="并发工作线程数。")
