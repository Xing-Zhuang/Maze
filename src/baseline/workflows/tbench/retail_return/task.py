...
def task2_find_user(args, dag_id, extracted_info, backend_data):
    # --- 工具导入 ---
...
def task3_get_order_details(args, dag_id, extracted_info, backend_data):
    # --- 工具导入 --
...
def task4_execute_return(args, dag_id, extracted_info, order_details, user_details, backend_data):
    # --- 工具导入 ---
    import time
    start_time = time.time()  # 记录开始时间 
    import json
    ...
    try:
        ...
        if not item_ids_to_return:
            ...
            return {
                "dag_id": dag_id,
                "final_result": None,
                "status": f"在订单 {order_details.get('order_id')} 中未找到指定的退货商品: {item_names_to_return}",
                "start_time": start_time,
                "end_time": time.time()
            }
        ...
        from baseline.utils.tbench_tools.retail.return_delivered_order_items import ReturnDeliveredOrderItems
...
def task5_output_result(args, dag_id, final_result):
