...
def load_dags_from_jsonl(file_path: str) -> List[Dict[str, Any]]:
    ...
    try:
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                if line.strip() and line_num% 6== 0:
                    try:
    ...
    while running_dags:
        ...
        for uuid, dag_id in running_dags.items():
            ...
            if status_info:
                ...
                process= "0/1"
                if status.lower() == "finished" or status.lower() == "error":
                    process= "1/1"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--master_addr", default="localhost:5001", help="Master API 的地址 (IP:port)。")
    parser.add_argument("--proj_path", default="/root/workspace/d1ofm7k7v38s73bb10ag/AgentOS", help="项目根目录的绝对路径。")
    ...
    submission_payload = {
        ...
        "dag_types": ["reason"], # "speech", "vision", "file", "reason"
