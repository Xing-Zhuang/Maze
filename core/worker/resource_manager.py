...
class ComputeNodeResourceManager:
    ...
    def __init__(self)-> None:
        ...
        self.logger.info("======集群各node资源======")
        self.logger.info(self.node2avai_resources)
    ...
    def find_runner_key_for_gpu(self, node_id: str, gpu_index: int) -> Optional[Tuple[str, frozenset]]:
        """
        根据单个GPU的ID，查找其所属的整个vLLM Runner部署组的Key。
        """
        with self.lock:
            try:
    ...
    def update_gpu_state(self, node_id: str, gpu_indices: List[int], state_updates: dict):
        ...
        with self.lock:
            try:
                # state_updates 中可以包含一个 'runner_key' 来标识部署组
                ...
                for gpu_index in gpu_indices:
                    ...
                    if runner_key:
                        gpu_to_update["runner_key"] = runner_key # 部署组

                self.logger.debug(f"🔄 GPU State Updated: Node {node_id[:6]}, GPUs {gpu_indices} -> {state_updates}")
            except (KeyError, IndexError) as e:
                self.logger.error(f"❌ Error updating GPU state for Node {node_id}, GPUs {gpu_indices}: {e}")
    ...
    def find_lru_runner(self, idle_gpu_candidates: List[Dict]) -> Optional[Tuple[str, List[int], str]]:
        """
        从一组闲置的GPU中，找到最久未被使用的那个模型部署实例 (Runner)。
        返回 (node_id, [gpu_indices], model_name)
        """
        if not idle_gpu_candidates:
            return None
        # 步骤1: 按 runner_key 将GPU分组
        runners = {}
